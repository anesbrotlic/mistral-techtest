...
s => s.MapFrom(m => string.Join(',', m.MovieActors.Select(ma => ma.Actor.Name).ToList())))
                .ForMember(d => d.AvgRating, s => s.MapFrom(m1 => Decimal.Round((decimal)m1.MovieRatings.Average(m => m.Rating), 1)))
            ...
            CreateMap<Movie, MovieDetailModel>()
                ...
                .ForMember(d => d.AvgRating, s => s.MapFrom(m => Decimal.Round((decimal)m.MovieRatings.Average(mr => mr.Rating), 1)))
                .ForMember(d => d.RatingByUser, s => s.MapFrom(m => 0))
                .ForMember(d=>d.ReleaseDate,s=>s.MapFrom(m=>m.ReleaseDate.ToShortDateString()));
